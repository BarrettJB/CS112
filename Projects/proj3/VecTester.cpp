...
void VecTester::testAddition() const {
	...
	v1.setItem(0, 1);
	v1.setItem(1, 2);
	v1.setItem(2, 3);	testWriteToFile();
	v2.setItem(0, 2);
	v2.setItem(1, 4);
	v2.setItem(2, 6);
...
void VecTester::testBracketAssign() const {
	cout << "Testing Item& operator[]... " << flush;
	// empty case
	Vec v0(1);
	v0[0] = 11;
	assert (v0.getItem(0) == 11);
	// nonempty case, valid subscript
	Vec v(5);
	for (unsigned i = 0; i < 5; i++) {
		v[i] =  (i+1);
...
void VecTester::testBracketRead() const {
	cout << "Testing Item operator[]... " << flush;
	Vec v(5);
	for (unsigned i = 0; i < 5; i++) {
		v[i] = (i+1);
